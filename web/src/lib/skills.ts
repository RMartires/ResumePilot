...
const CATEGORY_RULES = [
  {
    category: "Languages",
    match: /^(java|python|c\+\+|c$|javascript|typescript|bash|shell|\.net|go|ruby|php|kotlin|swift)/i,
  },
  {
    category: "Libraries",
    match: /(react|angular|express|django|spring|next\.?js|node\.?js|selenium)/i,
  },
  {
    category: "Tools",
    match: /(aws|azure|docker|kubernetes|sql|linux|unix|git|microservices|agentic)/i,
  },
];
...
export function parseSkillsString(raw: string): SkillEntry[] {
  if (!raw.trim()) return [];

  const entries: SkillEntry[] = [];
  const seen = new Set<string>();

  const addEntry = (name: string, years: number | null) => {
...
export function serializeSkills(entries: SkillEntry[], grouped: boolean): string {
  if (!entries.length) return "";

  const format = (entry: SkillEntry) => {
  ...
  const buckets: Record<string, SkillEntry[]> = {
    Languages: [],
    Libraries: [],
    Tools: [],
  };
...
export function parseSkillInput(input: string): SkillEntry | null {
...
export function hasSkill(entries: SkillEntry[], name: string): boolean {
