...
Unowned (or only weakly covered) terms. Re-score volume and competition with Keywords Everywhere in Step 1 before picking. Do not pick anything whose **cluster** is in cooldown — see [ship-log.md](./ship-log.md).

Status: `open` = fair game · `blocked` = owned/synonym/cooldown · `needs-page` = no honest URL yet.
...
| chronological vs functional resume | `ats-layout` | **Blocked until ats-layout cooldown ends** | format guide |
| can ats read pdf | `ats-layout` | **Blocked until ats-layout cooldown ends** | format guide |
| two page resume | `ats-layout` | **Blocked until ats-layout cooldown ends** | format guide |
| ats friendly cover letter | `cover-letter` | Feature page is thin vs how-to demand | `/features/cover-letter` or new guide |
| how to write an ats friendly resume | `how-to-write` | Close to the pillar; only pick if SERP is ATS-specific and pillar cannot rank it | pillar first; else new ATS how-to |

## Do not pick (already have a page)
...
Add leftover competitor-domain keywords here after each Step 1. Include KE volume, competition, cluster, and date seen.
